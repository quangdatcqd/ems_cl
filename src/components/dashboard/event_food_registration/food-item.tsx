...
export default function FoodItem({ item, setOrders, orders }: TypeProps) {
    ...
    return (
        <div className="relative" unselectable="on" onClick={onSelectDish}>
            ...
            <div className={`${isSelect ? "border-green-300" : ""} h-full select-none group border rounded-xl  overflow-hidden shadow-lg  cursor-pointer  relative   hover:-translate-y-1 transition-all ease-in-out duration-300 `}>
                <img
                ...
                <div className="p-3 pt-2 pb-4">
                    ...
                    <p className="font-[500] text-green-700 mt-1 text-sm">${item.price}</p>
                </div>
            </div>
        </div>
    )
